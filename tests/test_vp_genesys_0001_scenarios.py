"""Tests for VP-GENESYS-0001 scenario regression pack."""
...
class TestVpGenesys0001Scenarios:
    ...
    @pytest.mark.parametrize("filename", [name for _, name in GENESYS_EVIDENCE_FILES])
    ...
    def test_full_lifecycle_with_report_and_learning(self) -> None:
        ...
        try:
            ...
            for command, filename in GENESYS_EVIDENCE_FILES:
                submit_evidence(
                    case,
            ...
            assert "OAuth failure" in markdown
            assert closed_case.learning_record is not None
        finally:
            runtime.shutdown()

    def test_validate_command_supports_genesys_playbook(self) -> None:
    ...
    def test_discovery_plan_for_partial_evidence_scenario(self) -> None:
        ...
        try:
            ...
            submit_evidence(
                ...
                runtime.case_manager,
                "organization-export",
                (SCENARIOS_ROOT / "oauth_failure" / "organization_export.csv").read_text(),
            )
            runtime.analyze_case(case.case_id)
