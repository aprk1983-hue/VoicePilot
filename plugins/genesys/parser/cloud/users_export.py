"""Genesys Cloud ``users-export`` evidence parser."""
...
PARSER_ID = "genesys_users_export"
PARSER_VERSION = "1.0.0"


class GenesysUsersExportParser(GenesysCloudParser):
    vendor = VENDOR
    command = COMMAND
    parser_version = PARSER_VERSION
    detect_markers = ('user_id', 'email', 'username')
    identity_fields = ('user_id', 'id')
    ...
    def extract_findings(self, structured_data: JsonDict, context: ParserContext) -> list[ParserFinding]:
        ...
        for record in self._records(structured_data):
            ...
            state = (record_value(record, "state", "status", "presence") or "").lower()
            if state in {"offline", "logged_out", "not_logged_in"}:
    ...
    def extract_voice_objects(
        ...
        *,
        ...
        for record in self._records(structured_data):
            user_id = record_identity(record, "user_id", "id")
            objects.append(Agent.create(
                ...
                agent_id=user_id, user_id=user_id,
                display_name=record_value(record, "display_name", "name"),
                state=record_value(record, "state", "status"),
                queue_id=record_value(record, "queue_id", "assigned_queue"),
                name=record_value(record, "display_name", "name") or user_id, confidence=confidence, metadata=dict(record),
            ))
